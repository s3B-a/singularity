...
// Optimized AES implementation that utilizes hardware acceleration
...
impl AesOptimized {
    ...
    /**
     * Creates a new AesOptimized instance with a given key, checking for hardware support
     * Args:
     *    key - &[u8]: The encryption key (16, 24, or 32 bytes for AES 128, 192 or 256)
     * 
     * Returns:
     *    Result<Self>: The AesOptimized instance or an error if the key size is invalid
     */
    pub fn new(key: &[u8]) -> Result<Self> {
        let use_hardware = has_aes_ni();
        let key_size = key.len();
        if key_size != 16 && key_size != 24 && key_size != 32 {
            return Err(Error::InvalidKeySize);
        }

        let key_schedule = if use_hardware {
            Self::expand_key(key)
        } else {
            Self::expand_key(key)
        };
    ...
    /**
     * Encrypts a single 16-byte block using the appropriate method based on hardware support
     * Args:
     *    &self: The AesOptimized instance
     *    plaintext: &[u8; 16]: The 16-byte block to encrypt
     * 
     * Returns:
     *    [u8; 16]: The encrypted block
     */
    pub fn encrypt_block(&self, plaintext: &[u8; 16]) -> [u8; 16] {
        if self.use_hardware && cfg!(any(target_arch = "x86", target_arch = "x86_64")) {
            self.encrypt_block_aes_ni(plaintext)
        } else if self.use_hardware && cfg!(target_arch = "aarch64") {
            #[cfg(target_arch = "aarch64")]
            {
                self.encrypt_block_neon(plaintext)
            }
    ...
    /**
     * Encrypts a block using AES-NI instructions on x86/x86_64 platforms
     * UNTESTED ON x86/x86_64 PLATFORMS
     * Args:
     *    &self: The AesOptimized instance
     *    plaintext: &[u8; 16]: The 16-byte block to encrypt
     * 
     ...
     */
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    fn encrypt_block_aes_ni(&self, plaintext: &[u8; 16]) -> [u8; 16] {
        #[cfg(target_feature = "aes")]
        unsafe {
            use std::arch::x86_64::*;

            let state = _mm_loadu_si128(plaintext.as_ptr() as *const __m128i);
            let mut result = state;
            let rk0 = _mm_loadu_si128(
                self.key_schedule.as_ptr() as *const __m128i
            );

            result = _mm_xor_si128(result, rk0);
            let num_rounds = match self.key_size {
                16 => 10,
                24 => 12,
                32 => 14,
                _ => 10,
            ...
            for round in 1..num_rounds {
                let rk = _mm_loadu_si128(
                    self.key_schedule.as_ptr().add(round * 4) as *const __m128i
                );

                result = _mm_aesenc_si128(result, rk);
            }

            let final_rk = _mm_loadu_si128(
                self.key_schedule.as_ptr().add(num_rounds * 4) as *const __m128i
            );

            result = _mm_aesenclast_si128(result, final_rk);
    ...
    /**
     * Encrypts a block using NEON AES instructions on ARM64 platforms
     * UNTESTED ON ARM64 PLATFORMS
     * Args:
     *    &self: the AesOptimized instance
     *    plaintext: &[u8; 16]: The 16-byte block to encrypt
     * 
     ...
     */
    #[cfg(target_arch = "aarch64")]
    fn encrypt_block_neon(&self, plaintext: &[u8; 16]) -> [u8; 16] {
        #[cfg(target_feature = "aes")]
        unsafe {
            use std::arch::aarch64::*;

            let state = vld1q_u8(plaintext.as_ptr());
            let mut result = state;
            let rk0_bytes = &self.key_schedule;
            result = veorq_u8(result, vld1q_u8(rk0_bytes.as_ptr() as *const u8));
            let num_rounds = match self.key_size {
                16 => 10,
                24 => 12,
                32 => 14,
            ...
            for round in 1..num_rounds {
                let rk = vld1q_u8(
                    rk0_bytes.as_ptr().add(round * 4) as *const u8
                );
                
                result = vaesq_u8(result, rk);
            }

            let final_rk = vld1q_u8(
                rk0_bytes.as_ptr().add(num_rounds * 4) as *const u8
            );

            result = vaesq_u8(result, final_rk);
    ...
    /**
     * Encrypts a block using a pure software implementation of AES
     * Args:
     *    &self: The AesOptimized instance
     *    plaintext: &[u8; 16]: The 16-byte block to encrypt
     * 
     ...
     *    [u8; 16]: The encrypted block
     */
    fn encrypt_block_software(&self, plaintext: &[u8; 16]) -> [u8; 16] {
        let nk = self.key_size / 4;
        let mut key = vec![0u8; self.key_size];
        for i in 0..nk {
            let word = self.key_schedule[i];
            let bytes = word.to_be_bytes();
            key[i * 4..i * 4 + 4].copy_from_slice(&bytes);
        }

        let aes = Aes::new(&key).expect("Failed to create AES");
        aes.encrypt_block(plaintext)
    }

    /**
     * Expands the encryption key into a key schedule for AES-NI or NEON
     * Args:
     *    key - &[u8]: The encryption key to expand
     * 
     * Returns:
     *    Vec<u32>: The expanded key schedule
     */
    fn expand_key(key: &[u8]) -> Vec<u32> {
        match key.len() {
            16 => Self::expand_key_128(key),
            24 => Self::expand_key_192(key),
            32 => Self::expand_key_256(key),
            _ => Vec::new(),
        }
    }

    /**
     * Expands a 128-bit key into a key schedule for AES-NI or NEON
     * Args:
     *    key - &[u8]: The 16-byte encryption key to expand
     * 
     * Returns:
     *    Vec<u32>: The expanded key schedule
     */
    fn expand_key_128(key: &[u8]) -> Vec<u32> {
    ...
    /**
     * Expands a 192-bit key into a key schedule for AES-NI or NEON
     * Args:
     *    key - &[u8]: The 24-byte encryption key to expand
     * 
     * Returns:
     *    Vec<u32>: The expanded key schedule
     */
    fn expand_key_192(key: &[u8]) -> Vec<u32> {
        let mut w = Vec::with_capacity(52);
        for i in 0..6 {
            w.push(
                (key[i * 4] as u32) << 24
                | (key[i * 4 + 1] as u32) << 16
        ...
        let mut rcon_idx = 1;
        while w.len() < 52 {
            let mut temp = w[w.len() - 1];
            if w.len() % 6 == 0 {
                temp = ((temp << 8) | (temp >> 24)) & 0xFFFFFFFF;
                temp = Self::sub_word(temp);
                temp ^= Self::rcon(rcon_idx) as u32;
                rcon_idx += 1;
            }

            w.push(w[w.len() - 6] ^ temp);
        }
    ...
    /**
     * Expands a 256-bit key into a key schedule for AES-NI or NEON
     * Args:
     *    key - &[u8]: The 32-byte encryption key to expand
     * 
     * Returns:
     *    Vec<u32>: The expanded key schedule
     */
    fn expand_key_256(key: &[u8]) -> Vec<u32> {
        let mut w = Vec::with_capacity(60);
        for i in 0..8 {
            w.push(
                (key[i * 4] as u32) << 24
                | (key[i * 4 + 1] as u32) << 16
    ...
    /**
     * Applies the AES S-box to each byte of a 32-bit word
     * Args:
     *    word - u32: The 32-bit word to transform
     * 
     * Returns:
     *    u32: The transformed word
     */
    fn sub_word(word: u32) -> u32 {
        let b0 = ((word >> 24) & 0xFF) as u8;
        let b1 = ((word >> 16) & 0xFF) as u8;
        let b2 = ((word >> 8) & 0xFF) as u8;
        let b3 = (word & 0xFF) as u8;

        ((SBOX[b0 as usize] as u32) << 24)
            | ((SBOX[b1 as usize] as u32) << 16)
            | ((SBOX[b2 as usize] as u32) << 8)
            | (SBOX[b3 as usize] as u32)
    }

    /**
     * Returns the round constant for a given round number
     * Args:
     *    round - usize: The round number (1-based)
     * 
     * Returns:
     *    u8: The round constant for the given round
     */
...
fn mix_columns(state: &mut [u8; 16]) {
    for i in 0..4 {
        ...
        let t0 = T0[s0 as usize];
        let t1 = T1[s1 as usize];
        let t2 = T2[s2 as usize];
        let t3 = T3[s3 as usize];
        
        state[i * 4] = ((t0 ^ t1 ^ t2 ^ t3) & 0xFF) as u8;
        state[i * 4 + 1] = (((t0 ^ t1 ^ t2 ^ t3) >> 8) & 0xFF) as u8;
        state[i * 4 + 2] = (((t0 ^ t1 ^ t2 ^ t3) >> 16) & 0xFF) as u8;
        state[i * 4 + 3] = (((t0 ^ t1 ^ t2 ^ t3) >> 24) & 0xFF) as u8;
    }
}
