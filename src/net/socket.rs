...
const SOCKET_SECURE_FRAME_MAGIC: &str = "SINGULARITY_SOCKET_SECURE_FRAME_V1";
const SOCKET_SECURE_FRAME_CONTEXT: &str = "SINGULARITY_SOCKET_SECURE_FRAME_BINDING_V1";

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SecureSocketFrameMeta {
...
#[derive(Debug, Clone)]
pub struct SocketSecurityCfg {
    pub enable_integrity: bool,
    pub nonce_len: usize,
}

#[derive(Debug, Clone)]
pub struct SocketCompressionCfg {
    pub enabled: bool,
    pub algorithm: CompressionAlgorithm,
...
pub fn encode_secure_socket_frame(raw_payload: &[u8], security_cfg: &SocketSecurityCfg, compression_cfg: &SocketCompressionCfg) -> io::Result<(SecureSocketFrameMeta, Vec<u8>)> {
    let mut selected_algorithm = CompressionAlgorithm::Identity;
    if compression_cfg.enabled && raw_payload.len() >= compression_cfg.min_size && compression_cfg.algorithm.is_implemented() {
    ...
    let nonce = if security_cfg.enable_integrity {
        random::generate_random(security_cfg.nonce_len.max(8)).map_err(|e| {
            io::Error::new(
                io::ErrorKind::Other,
                format!("failed to generate secure socket nonce: {}", e),
            )
        })?
    } else {
        Vec::new()
    ...
    let meta = SecureSocketFrameMeta {
    ...
    let header = format!(
        ...
        magic = SOCKET_SECURE_FRAME_MAGIC,
...
pub fn decode_secure_socket_frame(secure_frame: &[u8], security_cfg: &SocketSecurityCfg) -> io::Result<(SecureSocketFrameMeta, Vec<u8>)> {
    ...
    if security_cfg.enable_integrity && !meta.integrity_enabled {
        return Err(io::Error::new(
            ...
            "received secure socket frame has integrity disabled",
        ));
    }
    ...
    if meta.integrity_enabled {
        let nonce = pem::decode(&meta.nonce_b64).map_err(|_| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                "invalid nonce in secure socket frame",
            )
        })?;
        ...
        let expected_tag = pem::decode(&meta.tag_b64).map_err(|_| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                "invalid tag in secure socket frame",
            )
        })?;
        ...
        if !constant_time_eq(&expected_tag, &computed_tag) {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "secure socket frame integrity verification failed",
            ));
        }
    }
    ...
    if decoded_payload.len() != meta.raw_size {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "secure socket frame size mismatch: expected {}, got {}",
                meta.raw_size,
                decoded_payload.len()
            ),
        ));
    ...
    let expected_digest = pem::decode(&meta.digest_b64).map_err(|_| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            "invalid digest in secure socket frame",
        )
    })?;
    ...
    if !constant_time_eq(&expected_digest, &computed_digest) {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "secure socket frame digest verification failed",
        ));
    }
...
impl Socket {
    ...
    pub fn send_secure(&mut self, payload: &[u8], security_cfg: &SocketSecurityCfg, compression_cfg: &SocketCompressionCfg) -> io::Result<SecureSocketFrameMeta> {
        let (meta, frame) = encode_secure_socket_frame(payload, security_cfg, compression_cfg)?;
        if frame.len() > u32::MAX as usize {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "secure socket frame too large",
            ));
        }
    ...
    pub fn send_secure_auto(&mut self, payload: &[u8], accept_encoding: &str) -> io::Result<SecureSocketFrameMeta> {
        let algorithm = select_secure_socket_compression_algorithm(accept_encoding);
        let mut compression_cfg = SocketCompressionCfg::default();
        compression_cfg.algorithm = algorithm;
        self.send_secure(payload, &SocketSecurityCfg::default(), &compression_cfg)
    }
    ...
    pub fn recv_secure(&mut self, security_cfg: &SocketSecurityCfg, max_frame_size: usize) -> io::Result<(SecureSocketFrameMeta, Vec<u8>)> {
        ...
        if frame_len == 0 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "secure socket frame length cannot be zero",
            ));
        }
        ...
        if frame_len > max_frame_size {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "secure socket frame exceeds maximum size: {} > {}",
    ...
    pub fn recv_secure_default(&mut self, max_frame_size: usize) -> io::Result<(SecureSocketFrameMeta, Vec<u8>)> {
        self.recv_secure(&SocketSecurityCfg::default(), max_frame_size)
    }
}
...
impl Default for SocketSecurityCfg {
    fn default() -> Self {
        Self {
...
impl Default for SocketCompressionCfg {
    fn default() -> Self {
        Self {
            enabled: true,
...
fn compute_frame_tag(nonce: &[u8], algorithm: CompressionAlgorithm, raw_size: usize, encoded_payload: &[u8]) -> [u8; 32] {
    ...
    key_material.extend_from_slice(SOCKET_SECURE_FRAME_CONTEXT.as_bytes());
    ...
    signed.extend_from_slice(SOCKET_SECURE_FRAME_MAGIC.as_bytes());
...
fn split_header_body(data: &[u8]) -> io::Result<(String, &[u8])> {
    ...
    let split_pos = data.windows(DELIM.len()).position(|w| w == DELIM).ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            "secure socket frame header separator not found",
        )
    })?;
    ...
    let header = String::from_utf8(data[..split_pos].to_vec()).map_err(|_| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            "secure socket frame header is not valid UTF-8",
        )
    })?;
...
fn parse_secure_frame_meta(header: &str, body_len: usize) -> io::Result<SecureSocketFrameMeta> {
    let mut lines = header.lines();
    let magic = lines.next().unwrap_or_default();
    if magic != SOCKET_SECURE_FRAME_MAGIC {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "invalid secure socket frame magic",
        ));
    }
    ...
    for line in lines {
        if let Some(v) = line.strip_prefix("content-encoding=") {
            algorithm = CompressionAlgorithm::from_content_encoding(v.trim()).ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    "unsupported secure socket frame content-encoding",
                )
            })?;
    ...
    let digest_b64 = digest_b64.ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            "secure socket frame missing digest",
        )
    })?;
    ...
    let raw_size = raw_size.ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            "secure socket frame missing raw-size",
        )
    })?;
    ...
    let encoded_size = encoded_size.ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            "secure socket frame missing encoded-size",
        )
    })?;
    ...
    let issued_at_unix = issued_at_unix.ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            "secure socket frame missing issued-at",
        )
    })?;
    ...
    if encoded_size != body_len {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "secure socket frame body length mismatch: expected {}, got {}",
    ...
    if integrity_enabled && (nonce_b64.is_empty() || tag_b64.is_empty()) {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "secure socket frame integrity is enabled but nonce/tag is missing",
        ));
    }

    Ok(SecureSocketFrameMeta {
        algorithm,
        integrity_enabled,
        nonce_b64,
        digest_b64,
...
mod tests {
    ...
    #[test]
    fn test_select_secure_socket_compression_algorithm() {
        let selected = select_secure_socket_compression_algorithm("identity;q=0.1, br;q=1.0");
        assert_eq!(selected, CompressionAlgorithm::Brotli);

        let selected_identity = select_secure_socket_compression_algorithm("identity");
        assert_eq!(selected_identity, CompressionAlgorithm::Identity);
    }

    #[test]
    fn test_secure_socket_frame_roundtrip_identity() {
        let payload = b"hello secure socket".to_vec();
        let security_cfg = SocketSecurityCfg::default();
        let compression_cfg = SocketCompressionCfg {
            enabled: false,
        ...
        let (_, decoded) =
            decode_secure_socket_frame(&frame, &security_cfg).expect("decode must succeed");
        assert_eq!(decoded, payload);
    }

    #[test]
    fn test_secure_socket_frame_roundtrip_compressed() {
    ...
    #[test]
    fn test_secure_socket_frame_tamper_detected() {
        ...
        let (_, mut frame) = encode_secure_socket_frame(&payload, &security_cfg, &compression_cfg)
            .expect("encode must succeed");

        let last = frame.len().saturating_sub(1);
        frame[last] ^= 0x01;

        let result = decode_secure_socket_frame(&frame, &security_cfg);
        assert!(result.is_err());
    }

    #[test]
    fn test_secure_socket_frame_requires_integrity_when_enabled() {
        let payload = b"no-integrity payload".to_vec();
        let no_integrity_cfg = SocketSecurityCfg {
        ...
        let (_, frame) =
            encode_secure_socket_frame(&payload, &no_integrity_cfg, &compression_cfg)
                .expect("encode must succeed");

        let result = decode_secure_socket_frame(&frame, &SocketSecurityCfg::default());
        assert!(result.is_err());
    }
}